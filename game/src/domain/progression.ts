// Progression curves — TS port of game/js/mechanics/progression.js.
// Values byte-identical to the 2D version.

export type PlayerClass = 'knight' | 'paladin' | 'sorcerer' | 'druid';

const BASE_PLAYER_HP = 150;
const BASE_PLAYER_MANA = 10;
const BASE_PLAYER_CAPACITY = 400;

const CLASS_GROWTH: Record<PlayerClass, { hp: number; mana: number; capacity: number }> = {
...
const CLASS_MAGIC_LEVEL_GROWTH: Record<PlayerClass, number> = {
  knight: 0.12,
  paladin: 0.45,
  sorcerer: 1.0,
  druid: 1.0,
...
export function parseDamageRangeString(raw: unknown): { min: number; max: number } | null {
...
export function averageMagicWeaponHitPreview(
  damageRangeRaw: unknown,
  ml: number,
  playerLevel: number,
): number | null {
...
export function progressionStatsForLevel(level: number, playerClass: PlayerClass = 'knight'): LevelStats {
  const lv = Math.max(1, Number(level || 1));
  const growth = CLASS_GROWTH[playerClass] ?? CLASS_GROWTH.knight;
  const mlGrowth = CLASS_MAGIC_LEVEL_GROWTH[playerClass] ?? CLASS_MAGIC_LEVEL_GROWTH.knight;
  return {
